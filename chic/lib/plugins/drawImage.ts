...
const drawImage: ChicPlugin = {
  install: chic => {
     ...
     * @param  image */
    const drawImage: ChicPluginFunction<void> = (image: CanvasImageSource) => {
    ...
    const formatRGBA = (out: any, val: number, idx: number): string[] => {
      if (!(idx % 4)) out.push([]);
      const color: number[] = out.at(-1);
      color.push(val);
      if (color.length === 4) {
     ...
     * @param  image */
    const generateRGBAStrings = (image: CanvasImageSource): string[] => {
      const { height, width } = parseDimensions(image);
